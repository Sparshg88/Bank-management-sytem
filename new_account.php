...
<div class="card card-outline card-primary">
        ...
        <div class="container-fluid">
            <form id="account-form">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="account_number" value="<?php echo isset($account_number)? $account_number : '' ?>" required>
                </div>
                <hr>
                <div class="row">
                    ...
                    <div class="form-group col-sm-4">
                        ...
                        <input type="text" class="form-control" name="middlename" value="<?php echo isset($middlename)? $middlename : '' ?>" placeholder="(optional)">
                    </div>
                    <div class="form-group col-sm-4">
                ...
                <?php if(!isset($id)): ?>
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="pin" value="<?php echo isset($pin)? $pin : '' ?>" required>
                    </div>
                    <div class="form-group">
                        <label class="control-label">Beginning Balance</label>
                        <input type="number" step="any" min="0" class="form-control text-right" name="balance" value="0" required>
                    </div>
                <?php endif; ?>
            </form>
        </div>
    ...
    <div class="card-footer">
        <div class="d-flex w-100 justify-content-end">
            ...
            <a href="./?page=accounts" class="btn btn-secondary">Cancel</a>
        </div>
    </div>
</div>
...
<script>
    $(function(){
        ...
        $('[name="account_number"]').on('input',function(){
            ...
            checks.text("Checking availability") 
            $('[name="account_number"]').after(checks)
            $.ajax({
                url:base_url+'classes/Master.php?f=check_account',
                method:'POST',
                ...
                success:function(resp){
                    if(resp.status == 'available'){
                        ...
                        checks.text('Account already exists')
        ...
        $('#account-form').submit(function(e){
            ...
            if($('.err_msg').length > 0)
                $('.err_msg').remove()
            $.ajax({
                url:base_url+'classes/Master.php?f=save_account',
                method:'POST',
                data:$(this).serialize(),
                dataType:'json',
                error:err=>{
